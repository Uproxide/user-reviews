...
class ReviewCell : public CCLayerColor {
    ...
    public:
        ...
        void onDelete(CCObject* sender) {
            this->m_listener.bind([this] (web::WebTask::Event* e) {
                if (web::WebResponse* res = e->getValue()) {
                    ...
                    log::info("deleting review progress: {}", p->downloadProgress().value_or(0.f));
                } else if (e->isCancelled()) {
                    log::info("The request was cancelled... So sad :(");
                }
            });
            web::WebRequest req = web::WebRequest();
            this->m_listener.setFilter(req.get(fmt::format("https://uproxide.xyz/api/v1/reviews/deleteReview.php?id={}", reviewsID)));
        }
